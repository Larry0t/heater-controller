// Three-phase heater controller with manual override ON = Force all relays ON (Emergency Heating Mode) (1.61)
...
if (m && m.topic) {
  ...
  // Force all relays ON - manual heating mode.
  else if (t === "/Heater/Manual") {
    cfg.manualOverride = msg.payload === 1;
    node.status({fill: cfg.manualOverride ? "yellow" : "green", shape: "dot", text: cfg.manualOverride ? "MANUAL: FORCE ON" : "AUTO"});
    return [null, null, null, { manualOverride: cfg.manualOverride }];
  }
}
...
// Force all relays ON - heat regardless of PV/battery logic later.
if (cfg.manualOverride) {
    // Force all relays ON regardless of logic
    const relays = [1, 1, 1];
